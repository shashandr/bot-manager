...
import { splitFirst, vcfExtractPhone } from '../../lib/strings.js';
import { downloadToTemp } from '../../lib/files.js';
...
export class MaxBot extends BaseBot {
    ...
    async onSubscribe(path) {
        const api = this.instance?.api;
        const payload = {
            url: path,
            update_types: ['bot_started', 'message_created', 'message_callback'],
        };
        if (api && typeof api.createSubscription === 'function') {
            await api.createSubscription(payload);
            return;
        }
        if (api && typeof api.createSubscriptions === 'function') {
            await api.createSubscriptions(payload);
            return;
        }
        ...
        if (api && api.subscriptions && typeof api.subscriptions.create === 'function') {
            await api.subscriptions.create(payload);
            return;
        }
        throw new Error('Max Bot API does not expose subscriptions API (createSubscription/subscribe)');
    }
