...
export class Bot {
    ...
    async handleEvent(eventName, payload) {
        ...
        try {
            ...
            throw new Error(`Handle event '${eventName}' error for bot '${this.serviceName}:${this.name}': ${err.message}`);
        }
    }
    ...
    async handleWebhook(update, headers = {}) {
        if (!this.webhook) {
            return;
        }
        try {
            ...
            throw new Error(`Handle webhook error for bot '${this.serviceName}:${this.name}': ${err.message}`);
        }
    }
    getMediaType(fileName) {
        return getFileType(fileName);
    }
    async start(webhook, subscriptionOptions) {
        ...
        try {
            ...
            throw new Error(`Webhook start error for bot '${this.serviceName}:${this.name}': ${err.message}`);
        }
    }
}
